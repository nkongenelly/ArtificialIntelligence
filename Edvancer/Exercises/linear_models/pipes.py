...
class cyclic_features_custom(BaseEstimator,TransformerMixin):
    ...
    def fit(self,x,y=None):
        ...
        for col in x.columns:
            ...
            self.feature_names.extend([col+'_week'+temp for temp in ['_sin','_cos']])
        return self
    ...
    def transform(self,X):
        ...
        for col in X.columns:
            ...
            X[col+'_week_sin']=np.sin(2*np.pi*wdays/self.week_freq)
            X[col+'_week_cos']=np.cos(2*np.pi*wdays/self.week_freq)
